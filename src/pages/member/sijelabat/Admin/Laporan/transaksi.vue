...
export default {
  ...
  data () {
    return {
      ...
      optionsRole: [
        { kode: 'PUBB', label: 'Bahan Baku' }, { kode: 'PUPN', label: 'Produsen' }, { kode: 'PUDR', label: 'Distributor' },
        { kode: 'PUKS', label: 'Kios' }
      ],
  ...
  methods: {
    getTransaksi () {
      if (this.formFilter.status === null || this.formFilter.role === null || this.formFilter.tanggalDari === null || this.formFilter.tanggalSampai === null) {
        this.$notif('Mohon Isi Semua Form Filter', 'negative')
        this.formFilter.status = null
        this.formFilter.role = null
      } else {
        this.formFilter.role = this.formFilter.role.kode
        this.formFilter.status = this.formFilter.status.kode
        this.$showLoading()
        this.$axios.post('sijelabat/admin-sijelabat/laporan/data-transaksi/show', this.formFilter, this.$createToken())
          .finally(() => this.$hide())
          .then(res => {
    ...
    toDetail (idorder) {
      ...
      this.$router.push({ name: 'AdminSejelabatTransaksiDetail', params: { idtr: ids } })
    }
  },
  created () {
    // this.getTransaksi()
