          ...
          <template v-slot:body-cell-action="props">
            <q-td :props="props">
              <div class="q-gutter-sm">
                <q-btn outline color="blue" size="sm" @click="toDetail(props.row.profil_usaha_id)" icon="view_timeline"> Detail</q-btn>
              </div>
            </q-td>
          </template>
        </q-table>
...
export default {
  ...
  methods: {
    getPU () {
      this.$showLoading()
      this.$axios.get('sijelabat/admin-sijelabat/profil-usaha/show/', this.$createToken())
        .finally(() => this.$hide())
        .then(res => {
