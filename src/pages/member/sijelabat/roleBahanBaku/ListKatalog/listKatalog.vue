          ...
          <template v-slot:body-cell-status="props">
            <q-td :props="props">
              <div class="q-gutter-sm">
                <q-toggle v-model="props.row.master_barang_pabrik_bahan_baku[0].status" @update:model-value="handleToggleUpdate(props.row.mm_id,  props.row.master_barang_pabrik_bahan_baku[0].status)" color="green" />
              </div>
            </q-td>
          </template>
...
export default {
  ...
  methods: {
    simpanKatalog () {
      ...
      this.$showLoading()
      if (this.modeEdit) {
        this.$axios.put(`sijelabat/bahan-baku/list-katalog/update/${btoa(this.idmm)}/`, formData, this.$createToken())
          .finally(() => this.$hide())
          .then(res => {
            // console.log(res)
        ...
        this.$axios.post('sijelabat/bahan-baku/list-katalog/insert/', formData, this.$createToken())
          .finally(() => this.$hide())
          .then(res => {
            // console.log(res)
            if (this.$parseResponse(res.data)) {
    ...
    getListKatalog () {
      this.$showLoading()
      this.$axios.get(`sijelabat/bahan-baku/list-katalog/show/${btoa(profile.member.id_member)}/${btoa(this.$route.params.idLayanan)}`, this.$createToken())
        .finally(() => this.$hide())
        .then(res => {
    ...
    toDetail (data) {
      ...
      this.modeEdit = true
      this.form = data
      this.form.profil_usaha_id = data.master_barang_pabrik_bahan_baku[0].profil_usaha_id
    ...
    handleToggleUpdate (newValue, status) {
      ...
      this.$showLoading()
      this.$axios.put(`sijelabat/bahan-baku/list-katalog/update-status/${btoa(newValue)}`, { status: status }, this.$createToken())
        .finally(() => this.$hide())
        .then(res => {
          if (this.$parseResponse(res.data)) {
            this.getListKatalog()
