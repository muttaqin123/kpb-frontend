...
export default {
  ...
  methods: {
    getSummary () {
      this.$axios.get(`sijelabat/bahan-baku/summary-transaksi/show/${btoa(profile.member.id_member)}/${btoa(this.$route.params.idLayanan)}`, this.$createToken())
        .then(res => {
    ...
    getTransaksi (status) {
      ...
      this.$showLoading()
      this.$axios.post(`sijelabat/bahan-baku/list-transaksi/show/${btoa(profile.member.id_member)}/${btoa(this.$route.params.idLayanan)}`,
        {
          status: data
        }, this.$createToken())
        .finally(() => this.$hide())
