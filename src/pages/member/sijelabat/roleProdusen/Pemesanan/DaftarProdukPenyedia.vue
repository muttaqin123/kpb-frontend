...
export default {
  ...
  methods: {
    getProduk () {
      this.$showLoading()
      this.$axios.get(`sijelabat/produsen/list-penyedia/produk/${this.$route.params.id}`, this.$createToken())
        .finally(() => this.$hide())
        .then(res => {
    ...
    toPayment () {
      ...
      this.$axios.post('sijelabat/checkout/keranjang/input/', {
        ...
        .then(res => {
          if (this.$parseResponse(res.data, false)) {
            this.$router.push({ name: 'sijelabatCheckoutProdukPemesananprodusen', params: { id: this.$route.params.id } })
          }
        }).catch(() => this.$commonErrorNotif())
    },
    getSelectedString () {
