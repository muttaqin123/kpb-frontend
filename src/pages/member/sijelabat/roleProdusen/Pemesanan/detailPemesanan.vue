...
export default {
  ...
  methods: {
    ...
    getDetailRiwayat () {
      this.$showLoading()
      this.$axios.get(`sijelabat/produsen/riwayat-pemesanan/detail/${this.$route.params.id}`, this.$createToken())
        .finally(() => this.$hide())
        .then(res => {
          // console.log(res)
    ...
    progressBayar (status) {
      if (status) {
        ...
        if (emptyFields.length > 0) {
          ...
          this.$showLoading()
          this.$axios.put(`sijelabat/produsen/riwayat-pemesanan/unggah-bayar/${this.$route.params.id}`, formData, this.$createToken())
            .finally(() => this.$hide())
            .then(res => {
