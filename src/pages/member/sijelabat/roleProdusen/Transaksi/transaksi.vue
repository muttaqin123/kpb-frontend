...
export default {
  ...
  methods: {
    getSummary () {
      this.$axios.get(`sijelabat/produsen/summary-transaksi/show/${btoa(profile.member.id_member)}/${btoa(this.$route.params.idLayanan)}`, this.$createToken())
        .then(res => {
    ...
    getTransaksi (status) {
      ...
      this.$showLoading()
      this.$axios.post(`sijelabat/produsen/list-transaksi/show/${btoa(profile.member.id_member)}/${btoa(this.$route.params.idLayanan)}`,
        {
          status: data
        }, this.$createToken())
        .finally(() => this.$hide())
    ...
    toDetail (idorder) {
      ...
      this.$router.push({ name: 'sijelabatDetailTransaksiprodusen', params: { id: ids } })
    }
  },
  created () {
    this.getTransaksi()
