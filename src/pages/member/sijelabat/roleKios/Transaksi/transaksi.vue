...
export default {
  ...
  methods: {
    getSummary () {
      this.$axios.get(`sijelabat/kios/summary-transaksi/show/${btoa(profile.member.id_member)}/${btoa(this.$route.params.idLayanan)}`, this.$createToken())
        .then(res => {
    ...
    getTransaksi (status) {
      ...
      this.$showLoading()
      this.$axios.post(`sijelabat/kios/list-transaksi/show/${btoa(profile.member.id_member)}/${btoa(this.$route.params.idLayanan)}`,
        {
          status: data
        }, this.$createToken())
        .finally(() => this.$hide())
    ...
    toDetail (idorder) {
      ...
      this.$router.push({ name: 'sijelabatDetailTransaksikios', params: { id: ids } })
    }
  },
  created () {
    this.getTransaksi()
