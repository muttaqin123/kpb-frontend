...
export default {
  ...
  methods: {
    getRiwayat () {
      this.$showLoading()
      this.$axios.get(`sijelabat/distributor/riwayat-pemesanan/get/${btoa(profile.member.id_member)}/${btoa(this.$route.params.idLayanan)}/${btoa(this.$route.params.idRole)}`, this.$createToken())
        .finally(() => this.$hide())
        .then(res => {
    ...
    toDetail (idtr) {
      const ids = btoa(idtr)
      this.$router.push({ name: 'sijelabatDetailPemesanandistributor', params: { id: ids } })
    },
    toPenyediaPemesanan () {
      this.$router.push({ name: 'sijelabatTambahPemesanandistributor' })
    }
  },
  created () {
    this.getRiwayat()
