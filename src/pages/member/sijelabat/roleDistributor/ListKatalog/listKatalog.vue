          ...
          <template v-slot:body-cell-status="props">
            <q-td :props="props">
              <div class="q-gutter-sm">
                <q-toggle v-model="props.row.master_barang_distributor_jelabat[0].status" @update:model-value="handleToggleUpdate(props.row.mm_id, props.row.master_barang_distributor_jelabat[0].status)" color="green" />
              </div>
            </q-td>
          </template>
...
export default {
  ...
  methods: {
    getListProfilUsaha () {
      this.$axios.get(`sijelabat/distributor/list-profil-usaha-produsen/byidmember/${btoa(profile.member.id_member)}/${btoa(this.$route.params.idLayanan)}`, this.$createToken())
        .then(res => {
    ...
    getListMM (val) {
      ...
      this.form.master_produsen = val.id
      const idprofilusaha = btoa(this.profil_usahaprodusen.profil_usaha_id)
      this.$axios.get(`sijelabat/distributor/list-mm-produsen/byidusaha/${idprofilusaha}`, this.$createToken())
        .then(res => {
    ...
    simpanKatalog () {
      ...
      this.$showLoading()
      if (this.modeEdit) {
        this.$axios.put(`sijelabat/distributor/list-katalog/update/${btoa(this.idmm)}/`, this.form, this.$createToken())
          .finally(() => this.$hide())
          .then(res => {
            // console.log(res)
        ...
        this.$axios.post('sijelabat/distributor/list-katalog/insert/', this.form, this.$createToken())
          .finally(() => this.$hide())
          .then(res => {
            // console.log(res)
            if (this.$parseResponse(res.data)) {
    ...
    getListKatalog () {
      this.$showLoading()
      this.$axios.get(`sijelabat/distributor/list-katalog/show/${btoa(profile.member.id_member)}/${btoa(this.$route.params.idLayanan)}`, this.$createToken())
        .finally(() => this.$hide())
        .then(res => {
          if (res.data.code === 507) {
            this.$notif(res.data.message, 'negative')
            this.$router.push({ name: 'sijelabatProfilUsahadistributor' })
          } else {
    ...
    toDetail (data) {
      ...
      this.modeEdit = true
      this.form = data
      this.form.profil_usaha_id = data.master_barang_distributor_jelabat[0].profil_usaha_id
      ...
      this.form.keterangan = data.master_barang_distributor_jelabat[0].keterangan
      this.form.mm_image = data.mm_img
    },
    handleToggleUpdate (newValue, status) {
      // Handle perubahan nilai sakelar di sini
      this.$showLoading()
      this.$axios.put(`sijelabat/distributor/list-katalog/update-status/${btoa(newValue)}`, { status: status }, this.$createToken())
        .finally(() => this.$hide())
        .then(res => {
          if (this.$parseResponse(res.data)) {
            this.getListKatalog()
