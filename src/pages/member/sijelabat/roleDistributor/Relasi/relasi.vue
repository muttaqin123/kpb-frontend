...
export default {
  ...
  methods: {
    getRelasi () {
      this.$showLoading()
      this.$axios.get(`sijelabat/distributor/list-relasi/show/${btoa(profile.member.id_member)}/${btoa(this.$route.params.idLayanan)}`, this.$createToken())
        .finally(() => this.$hide())
        .then(res => {
    ...
    toDetail (idu) {
      const ids = btoa(idu)
      this.$router.push({ name: 'sijelabatDetailRelasidistributor', params: { id: ids } })
    }
  },
  created () {
    this.getRelasi()
