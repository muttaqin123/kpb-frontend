    ...
    >
      <template v-slot:top-right>
        <q-btn
          ...
          unelevated
          color="primary"
          :to="{name: 'inputPengaduanDPIPetani'}"
        />
      </template>
      <template v-slot:body="props">
        <q-tr :props="props">
          ...
          <q-td key="kategoridpi" :props="props">
          ...
          <q-td key="berkas" :props="props">
            <q-btn @click="openDialogBerkas(props.row.opt_jawaban_dpi)" label="Lihat Berkas" outline/>
          </q-td>
          <q-td key="jawaban" :props="props">
            <div class="justify-center q-gutter-x-xs">
              <q-btn :disable="props.row.opt_jawaban_dpi.length < 1" label="Lihat Jawaban" @click="lihatJawaban(props.row)" color="primary" outline/>
            </div>
          </q-td>
        </q-tr>
      </template>
        ...
        <q-card-section class="q-pt-none">
          <div class="row q-col-gutter-y-sm">
            <div class="col-md-6 col-sm-6 col-xs-12">
              <custom-list
                title="Dokter"
                :subTitle="jawaban.opt_dokter_opt_dokterToopt_jawaban_dpi_id_opt_dokter.ktp.nama"
              />
            </div>
            <div class="col-md-6 col-sm-6 col-xs-12">
              <custom-list
                title="Laboran"
                :subTitle="jawaban.opt_dokter_opt_dokterToopt_jawaban_dpi_id_opt_tenagalab.ktp.nama"
              />
            </div>
            <div class="col-md-6 col-sm-6 col-xs-12">
              <custom-list
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { name: 'kategoridpi', align: 'left', label: 'Kategori', field: 'kategoridpi' },
  ...
  methods: {
    getData () {
      this.loading = true
      this.$axios.get(`opt/pengaduan-dpi-nik/${this.$getProfile().nik}`, this.$createToken())
