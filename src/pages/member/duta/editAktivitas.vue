      ...
      <div class="q-mt-md column border-shadow q-pa-md">
        ...
        <q-form
          ...
          <div class="q-gutter-sm q-mt-sm">
            ...
            <q-file label="Foto Kegiatan" max-files="3" multiple v-model="images" outlined :rules="[ val => val !== null || 'Masukan minimal 1 Foto']">
              <template v-slot:prepend>
                <q-icon name="attach_file" />
              </template>
            </q-file>
            <q-editor v-model="form.narasi" />
            <q-btn label="Edit Kegiatan" class="q-mt-sm full-width" type="submit" no-caps color="primary"/>
          </div>
        </q-form>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    updateActivity () {
      ...
      this.$axios.put(`/duta/updateActivity/${this.$route.params.id}`, formData, this.$createToken())
        .finally(() => this.$hide())
        .then((res) => {
          if (res.data.status) {
